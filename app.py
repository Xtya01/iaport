...
def ia_list():
    try:
        ...
        if r.ok:
            return [{"name": f["name"], "size": int(f.get("size",0))} 
                    for f in r.json().get("files",[]) if not f["name"].startswith("_")]
    except: pass
    return []

@app.before_request
def check():
    if request.path not in ["/login"] and not session.get("ok"):
...
def home():
    ...
    return f'<h1>{IA_BUCKET}</h1><form action=/upload method=post enctype=multipart/form-data><input type=file name=file><button>Upload</button></form><ul>{rows}</ul>'
